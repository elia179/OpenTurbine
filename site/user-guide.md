...
8. [Dry testing](#dry-testing)
9. [Pre-fuel checks](#pre-fuel-checks)
...
Open **Hardware** before entering engine settings. Select the exact controller target, enable only devices that are physically fitted, choose each electrical type, and assign each GPIO once. Save and allow the controller to reboot, then reopen Hardware and compare the saved configuration with the wiring.
...
In **Config - Setup**, enter limits only from authoritative engine, sensor, and actuator information. Review selected temperature source, RPM limits, oil targets, throttle behavior, flameout behavior, and each fitted optional safety. A visible example or suggested value is not an approved setting for a particular engine.

Settings that require unavailable hardware are hidden in Setup. Advanced view exposes deeper applicable controls, but it does not make an unfitted sensor or driver safe to use.
...
Download the complete engine file from **Tools** before significant configuration changes and before updates. It can contain hardware settings, calibration, sequences, logs, and Wi-Fi credentials. Store it securely and remove credentials before sharing diagnostics.
